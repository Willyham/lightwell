...
- **Keep automated app launches in the background.** Every automated editor launch, whether run by hand or through `verify`, is a hidden-window macOS background launch with a focus check that fails the run if the frontmost application changed. On macOS, use `verify`, `cargo xtask smoke`, `hardening`, `measure` or `probe`, which launch this way. For ad hoc API/render checks use `cargo xtask develop --background` with an isolated catalog or evidence directory. Do not launch the GUI binary directly or activate it through UI automation for routine tests. Foreground interaction checks require the owner's explicit request.
