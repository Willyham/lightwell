...
S0 (JPEG viewing) is accepted; the evidence harness now drives the editor itself. M1 (persistent history and live JSON API), M2 (exact rotate, flip and mirror) and M3 (tool modules with generated controls and API) are implemented and verified on the M4 Mac. M4 (Lightroom-style crop as a module with drafts, conflicts and API parity) is implemented and verified on the M4 Mac, as are content-space edits (pixel-stage layers placed before the geometry tail) and the single orientation layer for exact transforms. The Develop workspace shell (widget crate, layered desktop, generated panels, canvas modes, palette) is implemented and verified on the M4 Mac over those modules, and so are the Basic adjustments (one colour-stage layer: white balance with a neutral picker, exposure, tone, vibrance and saturation, with the core draft lifecycle behind the slider gesture) and the histogram inspector with clipping overlays and pixel readout. Initial continuous RAW editing for Nikon Z6 lossless NEF and Fujifilm X100VI uncompressed/lossless RAF is implemented, with source exposure/WB, retained sensor/float data and shared UI/API history. The supplied DJI Air 2S FC3411 uncompressed DNG is also supported with required gain-map/chromatic-warp corrections and continuous source editing. Instant previews are implemented and verified: at Fit every preview job renders a display-bounded proxy first and presents it through a surface that owns its texture, with the exact render as a cancellable second phase behind the histogram, the overlays and the 100% view ([design](docs/design/instant-preview.md)). Presence (texture, clarity, dehaze on a tiled spatial primitive), the HSL colour mixer and the post-crop vignette are implemented and verified on the M4 Mac, with the spatial and finish stages and an order within a stage in the host. Presets are implemented and verified on the M4 Mac: named settings sets in the catalog applied as one history entry by the presets module through a host composite plan, with Lightroom XMP and `.lrtemplate` import that transfers the values Lightwell has controls for and reports the rest ([design](docs/design/presets.md)). The state panel's Performance section is implemented and verified on the M4 Mac: the editor's memory, CPU and GPU with one-minute sparklines and its long-running work, read from a host activity board and process resource counters that clients also reach through `activity.list` and `resources.read` ([design](docs/design/performance-panel.md)). Controlled quality and resource/platform qualification remain tracked. Next: RAW qualification, export, Locate, MCP and full-editor verification. Everything is v0: no public compatibility framework, release planning, cloud, accounts, marketplace or generalized node graph. See the [roadmap](docs/plan.md) and [feature status](docs/features.md). Shared module capabilities are implemented and verified on the M4 Mac: user-level settings and provider profiles, Keychain secrets, per-asset consent grantable only with permission authority, a checked HTTPS/loopback transport, activation, verified resource installs and tasks on a bounded worker, and immutable derived artifacts beside the catalog (catalog format 6), proven by a developer-only proof module ([design](docs/design/module-capabilities.md)).
...
- **Verify proportionately.** Run the tiered `verify` command: `quick` before handing off any change, `rendered` and `timing` at integration points, `full` before a milestone claim ([tiers](docs/engineering/development.md#verification-tiers)). UI or image changes need a real rendered check with correlated state and logs. Changes under `crates/` answer the performance-rules checklist.
- **Keep automated app launches in the background.** A real launch of the editor takes focus like any application; an automated one never does. On macOS, `verify`, `cargo xtask smoke`, `hardening`, `measure` and `probe` launch a hidden window in a background-only bundle, so a run neither activates nor appears, whatever else is happening on the desktop. For ad hoc API/render checks use `cargo xtask develop --background` with an isolated catalog or evidence directory. Do not launch the GUI binary directly or activate it through UI automation for routine tests. Foreground interaction checks require the owner's explicit request.
