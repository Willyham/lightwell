...
- The first end-to-end build is S0: a cross-platform skeleton that opens and displays a JPEG at Fit. No editing tools, catalog, export or production MCP are required in S0. Verify launch/load on the accepted macOS/Windows/Linux matrix before marking S0 complete; M1 adds the editor.
- Prefer open-source-only project code and extensions. The owner has selected GPL-3.0-or-later. Apply it in TASK-005 and verify configured dependency compatibility in TASK-041; selection does not imply the audit is complete.
...
- Pin selected dependencies and inspect the licenses of the actual configured build, including native libraries and optional RAW components.
