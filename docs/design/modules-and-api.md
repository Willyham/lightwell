...
| `parameters[]` | `name`, `kind`, `required`, `default`, `unit`, `notes`. Kinds: `integer {min, max}`, `enum {options[]}`, `color` (three 8-bit sRGB channels) |
| `controls[]` | Ordered semantic controls: `group {label, controls[]}`, `number {action, parameter, label}`, `color {action, parameter, label}`, `action {action, label, preset}` where `preset` supplies fixed parameter values |
| `canvas` | Optional `point-pick {action, x, y}`: a pointer pick on the image fills the named integer parameters of that action; it never commits |
...
plan(&ActionInput, &StageContext)        -> ActionPlan::{NoOp, Commit(Layer)}
...
- `plan` sees the current output stage (`width`, `height`) and a point sampler over the current stack. It rejects out-of-stage coordinates and reports no-ops; it allocates no frame and mutates nothing.
...
Processing primitives are the closed host set for v0: `ExactGeometry` (an integer mapping with declared output size, composable into one raster pass) and `PointReplace` (one input-stage pixel, applied through the geometry that follows it). Color or interpolating stages are added to the host when the first tool needs them; there is no generalized graph.
...
5. `NoOp` records the request without a row. `Commit` validates the appended recipe through the registry and persists the snapshot, entry, current pointer, revision, cleared redo and request result in one transaction.

Rendering and point sampling resolve every layer's effect through the registry: `compile` runs per layer at its input stage, geometry composes into one mapping and point replacements map through the suffix geometry, exactly as before M3.

## Registry

`ModuleRegistry::builtin()` links the pixel and transform modules; `register` accepts any `ToolModule` and rejects: an invalid module, effect or action identity; a duplicate module, effect or action ID; a control naming an undeclared action or parameter; a preset or default outside the parameter's range; a parameter without a kind. Registration builds hash lookups from descriptors only and touches no image or catalog resource. A module that needs an expensive resource initializes it on first `compile` or `plan`; registration and first-use costs are measured separately.
