...
| `controls[]` | Ordered semantic controls: `group {label, controls[], reset?, collapsed?}`, `number {action, parameter, label, style?, rail?}` (integer or number; slider, field or stepper), `toggle {action, parameter, label}` (boolean), `choice {action, parameter, label, style?}` (enum), `color {action, parameter, label, style?}` (fields or picker), `curve {action, channels[{parameter, label}], label, sample_query, background?}` (curve parameters), and `action {action, label, preset?, style?, icon?}`. Omitted styles use the documented defaults. Rails are plain, hue, temperature, tint or a gradient of 2–8 RGB stops. A curve's `sample_query` declares each channel as an optional parameter without a default; the host sends only the active channel's points and uses the module's sampled result for the plot. Icon names are lowercase hyphenated; an unknown valid name falls back to the label |
