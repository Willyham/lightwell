...
Status: continuous RAW editing is implemented and verified on supplied files; broader qualification remains tracked. The editor opens the supplied Nikon Z6 and Fujifilm X100VI originals through the typed RAW path. Real-file adapter tests and background Metal renders pass; complete control, concurrency, resource and portability acceptance is tracked in [initial RAW](initial-raw.md) and the [task plan](../../tasks/implementation-initial-raw.json). The supplied DJI DNG is explicitly unsupported because its required GainMap and WarpRectilinear corrections are not implemented.
...
The supplied DJI FC3411 DNG has mandatory OpcodeList3 GainMap (9) and WarpRectilinear (1), both with the optional flag unset. It is rejected before unpack, and the editor retains the last successful photo. Generic DNG support is not implied by successfully unpacking its integer samples in an experiment.
