...
Status: **authorized by the owner on 2026-09-23 and in progress.** Implemented so far: the mask data model and its persistence (the recipe's mask table, the layer's mask reference, structural validation and retention of an unknown component kind, in catalog format 5 and recipe format 2), `render.transform`, the geometry tail as one affine, and the [`mask.*` host command family](#host-commands): every method of its table, declared with the same `ActionDescriptor`, `ParameterDescriptor` and `Control` types a module uses so one generic validation path and one control-generation path serve both, listed by `schema.list`, drafted through the delivered `draft.*` lifecycle, deduplicated and revision-checked like every other mutation, and labelled by the [granularity table](#history-granularity) below. The coverage mathematics is frozen by the [mask study](mask-study.md) against an independent `f64` reference, which settles [proposals](#proposals-with-recorded-defaults) P2 and P3 with figures. **Nothing yet evaluates a mask**, so no mask can affect a picture: coverage evaluation, the `mask` target on module actions, the masked colour and spatial primitives, the gradients, brushes, non-AI range selections and the Mask panel are the remaining phases. `mask.add-stroke` and `mask.delete-stroke` arrive with the brush in phase C. Local adjustments are delivered as a host concept over the delivered modules, on a [phased task plan](../../tasks/implementation-masking.json); the [proposals](#proposals-with-recorded-defaults) at the end carry the defaults the work runs on, each the owner's to refine. It builds on the [module and API contract](modules-and-api.md), the delivered [Basic adjustments](basic-and-histogram.md), [Presence, mixer and vignette](presence-mixer-vignette.md), the [content-space edit rule](content-space-edits.md), the [UI components](ui-components.md) vocabulary, the [Develop workspace](develop-workspace.md) tool array and the [instant preview](instant-preview.md) proxy contract.
