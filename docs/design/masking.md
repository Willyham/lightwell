...
- **Exactness.** Every coverage equation is transcribed twice — the production unit and an independent `f64` reference under `crates/luxforge-core/tests/reference/` — and tested to be bit-identical for the same inputs, as the vignette unit and its reference already are. Masked rendering is proved against stepwise references on synthetic fixtures: `M = 0` is the unmasked input byte for byte, `M = 1` is the unmasked effect byte for byte, and a half-covered fixture matches the blend computed independently.
