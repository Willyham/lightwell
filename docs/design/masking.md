...
Status: **authorized by the owner on 2026-09-23 and in progress.** Implemented so far: the mask data model and its persistence (the recipe's mask table, the layer's mask reference, structural validation and retention of an unknown component kind, in catalog format 5 and recipe format 2); `render.transform`, the geometry tail as one affine; the compiled mask and the linear gradient component; the `maskable` flag and the host-owned `mask` target field with per-target `single_layer` and the ordering rule below; and the masked colour primitive, so a masked Basic or colour-mixer layer changes a picture through both rendering paths and `render.sample` equals the rendered byte. The coverage mathematics is frozen by the [mask study](mask-study.md) against an independent `f64` reference, which settles [proposals](#proposals-with-recorded-defaults) P2 and P3 with figures. Still to come: the masked **spatial** primitive — until it lands a masked Presence layer is refused by name, never rendered as if it applied everywhere — the radial gradient, brushes, non-AI range selections, the `mask.*` commands, the Mask panel, and a mask target on a draft, so a masked slider gesture commits through the action rather than through `draft.*`. Local adjustments are delivered as a host concept over the delivered modules, on a [phased task plan](../../tasks/implementation-masking.json); the [proposals](#proposals-with-recorded-defaults) at the end carry the defaults the work runs on, each the owner's to refine. It builds on the [module and API contract](modules-and-api.md), the delivered [Basic adjustments](basic-and-histogram.md), [Presence, mixer and vignette](presence-mixer-vignette.md), the [content-space edit rule](content-space-edits.md), the [UI components](ui-components.md) vocabulary, the [Develop workspace](develop-workspace.md) tool array and the [instant preview](instant-preview.md) proxy contract.
