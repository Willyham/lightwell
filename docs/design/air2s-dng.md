...
- Optical correction rows use the shared Rayon pool for active areas of at least one megapixel and remain serial below it. Rows write disjoint slices; stage and channel joins preserve opcode order and the scalar f64 arithmetic. One active-area warp scratch plane is reused, with cancellation at row boundaries and copyback.
...
The existing [RAW pixel/color contract](initial-raw.md#pixel-and-color-contract) requires finite negative values and highlight headroom to survive until terminal display. Air 2S support inherits that contract; it does not introduce an early clipping stage. The supplied GainMap reaches roughly 4.1–4.7 depending on channel, so per-opcode clipping would discard detail before the existing exposure operation. The interpretation explicitly names unclipped evaluation. Independent references distinguish it from the normative clipped DNG variant; this is a neutral Luxforge development, not a strict Adobe rendering match.
