...
`reconstruct` is `from_oklab`, except that an achromatic Oklab colour (`a = b = 0`, either sign of zero) reconstructs to `L³` in all three channels. That is what the exact matrices give it: the first column of `M2⁻¹` is exactly one, so `(L, 0, 0)` maps to `LMS' = (L, L, L)`, and each row of `M1⁻¹` sums to one to ten decimals. The published rows are rounded, though, and in f32 the three products `M1⁻¹ · (x, x, x)` round differently: measured over 1 000 001 evenly spaced `L` in `[0, 1]`, `from_oklab(L, 0, 0)` returns channels up to `5.36e-7` apart and puts 8 of them on different sides of an output code threshold, a grey rendered with one channel a code off (`an_achromatic_colour_reconstructs_to_three_identical_channels`). The dedicated path gives three bit-identical channels. It lives in the mixer's own reconstruction; Basic's conversion is unchanged.
