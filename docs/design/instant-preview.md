...
Status: accepted design for the owner's requirement that a slider drag feels instant, as it does in Lightroom; implementation is tracked in [tasks](../../tasks/README.md). It changes how the desktop previews a stack while a gesture is open and how the Fit view is produced; it changes no recipe, history, API mutation or export behaviour, and the exact full-resolution render remains the only source of the histogram, the clipping counters and the 100% view. The measurements that motivated it and the ones that qualify it are in [performance](../specs/performance.md).
...
- One proxy source is cached per preview queue, keyed by the source identity (fingerprint, and for RAW the developed planes' allocation identity, so a white-balance redevelopment invalidates it), the proxy dimensions and the bounds. The cache is built on the preview worker on a miss and never on the owner thread. A window resize is a miss; the next job rebuilds it.
- Eligibility is decided by the registry: a recipe is proxy-eligible when every layer's effect is at the source, colour or geometry stage. A pixel-stage layer (the point-replacement proof module, whose coordinates are content pixels) makes the stack ineligible and the job takes the exact path, as does any compile failure at the proxy size. Ineligibility is reported in the result and the desktop's state summary, never silently.
...
- Events: `preview_displayed` carries `proxy: bool` and, when true, `proxy_dimensions`; `analysis_adopted` is unchanged; a new `preview_exact_cancelled` event counts superseded exact phases; the state summary carries `proxy: {eligible, dimensions, bounds}`.
