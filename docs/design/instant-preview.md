...
- The RAW retained mosaic and float development are unchanged; only their preview reads a smaller plane set. RAW white balance still redevelops the mosaic on the source worker, and only for a committed value: a drafted temperature or tint is accepted, but its preview job answers preparation-required, so a white-balance drag on a RAW source shows no frame until release. That is a recorded remaining cost, not part of this design.
...
- **RAW white balance drag.** A matrix approximation on the developed planes during the gesture, with the mosaic redevelopment on release, would make temperature and tint drags as fast as exposure on RAW. Today such a drag shows nothing until release. The non-approximate alternative, redeveloping the mosaic for each drafted value with at most one development in flight and the newest value waiting, would show a frame every redevelopment (about half a second on the Z6, one and a half on the X100VI) at the cost of one redevelopment per value the drag settles on; it is not built either.
