...
- Bounds: one proxy source (≤ 96 MiB without a crop, up to the 512 MiB frame limit under a tight one), one proxy raster per job, the cancellation token; no new timer, no private pool.
...
- **The `raw-panel` agreement check, relative to the drag (awaiting the owner).** The scenario asserts that the released exact frame is within one code, on average over the photo surface, of the drafted approximate frame, at Fit (as shot to 3500 K) and at 100% (3500 K to 2500 K). The Air 2S fails it at 100%, deterministically across three runs of the release build (host load average 21–29), while the design's own accuracy table already puts its full-size error above a code at a strong change (1.20 on average at 3200 K). Mean |Δ| over the surface, released against drafted and, for scale, released against the frame before the drag:
  ...
  The proposal: at both zooms, assert that the released frame's mean distance from the drafted frame is at most a tenth of its distance from the frame before the drag — the approximation removes at least 90% of the difference the drag is about, which is this design's claim — and keep the absolute one-code check at Fit, where every source passes it. A stale or wrong drafted frame is still caught: one that never moved is about 100% of the drag's difference. Rejected alternatives: a per-source threshold is a number fitted to one file; excluding bright or specular pixels leaves the Air 2S at 1.05; a median is 0 on every source and cannot see a wrong frame; and a gentler 100% drag would pass by avoiding the case the check exists for.
