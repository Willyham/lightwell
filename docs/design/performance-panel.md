...
Proposals, not decisions, until the owner answers:

- **Default state.** Proposed: collapsed on every launch, because the section's timer is the only way it costs anything and the idle target is already missed. The alternative is to remember the choice, which needs the host-owned app settings the [capability design](module-capabilities.md) proposes.
- **Units.** Proposed: binary sizes with Activity Monitor's MB and GB labels, and CPU as percent of one core, to match what a Mac user compares it with.
