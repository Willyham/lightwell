...
Status: implementation and qualification in progress. The owner authorized isolated worktrees and subagents for the measured opportunities. This batch preserves existing image and editing contracts; no approximation or GPU behavior is introduced.
...
2. Texture and Clarity declare that they need no global estimate. Dehaze's estimate identity excludes only its own strength; source, upstream recipe, stage and white-balance identities still invalidate it. Existing provider defaults remain safe and caches bounded.
...
5. Clipping reduction partitions disjoint output rows into one final grid rather than allocating and OR-merging a complete grid per parallel fold. Preserve exact cells, dimensions and resource limits. An additional persistent cache is outside this change.
...
Update performance results and current feature/user documentation only after evidence passes. Complete and remove the task plan once all accepted work is finished.
...
- Quantizer adds a bounded static table; DNG retains one bounded scratch plane; clipping removes transient grid copies; RAW row path adds no frame-sized scratch.
