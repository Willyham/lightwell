...
| Basic: Tone / Exposure | Tools panel, first section | module | Implemented: the generated slider drafts, previews live and commits once on release | `edit.set-basic`, `edit.reset-basic` and `draft.begin/set/commit/cancel/reapply` |
| Basic: the rest of Tone, White balance (Temperature, Tint, neutral picker), Color (Vibrance, Saturation) | Tools panel, first section | module + picker mode | Proposed ([Basic and histogram](basic-and-histogram.md)); no control exists | `edit.set-basic` as each parameter ships |
...
- The **histogram** sits above the first module section with no header of its own: a 96 pt plot of the three channels as filled, overlapping polygons on one shared linear vertical scale (the largest count in any channel), then a caption naming the domain ("Output · sRGB · after crop") with the pointer readout appended to it, then the endpoint counts in words for a reader who cannot measure the plot. It describes the rendered SDR sRGB output of the whole composition, after crop and edits, so panning, zooming and display scale never change its population. Pending, updating (a stale result still shown while a newer frame renders), and unavailable with its reason are explicit lines in place of the counts, never a silently empty plot. Two triangles sit in the bottom corners, shadow left and highlight right, tinted when their endpoint has pixels and filled when their overlay is on; clicking one toggles that overlay through `workspace.set`, and its tooltip states the rule. The counts come from the same preview worker that rendered the frame, under the same generation as the pixels, and are handed to the catalog owner so an API client's `analysis.request` for that identity is a cache hit rather than a second render.
...
- **Keyboard.** Cmd+O, Cmd+Z, Shift+Cmd+Z, Tab and Shift+Tab between fields, Enter and Escape in a draft, Space-drag, `F` Fit, `1` 100%, `\` compare (held), `O` thirds, `V` pointer, each module's declared letter (`R` crop), `J` both clipping overlays, Cmd+K command palette, Cmd+Option+[ and ] toggle the side panels, Up and Down and Enter in the palette. Letters act only when no text field has focus; the keymap is one table in `app/keymap.rs`. `W` neutral picker and Cmd+E export arrive with their features. Arrow keys on a focused slider are Iced's own stepping and are not covered by rendered evidence.
...
4. A slider drag sends one field message per pointer move and never a request from the move itself. What the gesture then does depends on the action the control belongs to, and the update function is where that bound lives.
   - A control of a **field-patch** action (`set-basic` today) drafts. The first move opens one core draft with `draft.begin`; later moves only replace a pending value. A 16 ms tick, gated on the open draft exactly as the preview poll is gated on an in-flight preview, sends at most one `draft.set` for the newest pending value together with the one preview job for the settings it accepted, and sends nothing at all while that round trip is in flight — so a drag of any length costs one `draft.set` and one preview per frame at most, and no `asset.state` or `history.list` request. Release sends one `draft.commit`, whose entry merges into history like any command's and whose no-op outcome ends the gesture with no entry and no history refresh.
   - Every other slider keeps the old rule: the drag changes the field and nothing else, and release, key-up or Enter sends the control's action once, exactly as Enter in a field does. Crop and pixel requests are unchanged.
   - What a control submits follows the same split. A generated control of a patch action submits its own parameter only and a reset or action control of one submits its declared preset only, so a Basic slider sends exactly one field; a control of any other action keeps sending every parameter the action declares. Double-clicking a label sets that field to its declared default, which on a patch action is one action submitting that field alone.
