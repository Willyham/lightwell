...
| Basic neutral picker (`W`) | Mode strip | mode | Implemented: a click samples the photograph and commits the white balance that neutralises it, once | `query.neutral-sample` at the located content pixel, then `edit.set-basic` |
| RAW sensor neutral picker (`N`) | Mode strip, RAW sources only | mode | Implemented: a click on a RAW photograph commits the sensor white balance that neutralises that pixel | `edit.pick-raw-neutral` at the located content pixel |
