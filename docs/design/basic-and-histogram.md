...
The first slice persists only its implemented exposure state. Later slices must explicitly define payload interpretation for older Basic histories (retained evaluator or proven conversion with neutral new fields). Do not prebuild dormant controls or reinterpret an older payload with changed mathematics. Internal effect format markers protect data; no public compatibility framework is needed.
...
- Match a separate f64 stepwise reference: proposed float tolerance `1e-6 + 1e-6 * abs(reference)` for supported non-clipped exposure/conversion cases, with at most one output code of rounding difference where permitted. Identity, old geometry/pixel fixtures and histogram counts remain exact. Freeze additional per-algorithm tolerances before implementation.
...
3. Complete history journey: gesture commit/cancel/reset, repeated values, retry/deduplication, undo/redo, preview/restore, reopen, failed write, unavailable provider, source missing/changed and retained pre-module fixtures. Originals remain byte-identical.
