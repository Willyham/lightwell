...
Status: frozen and implemented. The numerical study froze every constant and formula here ahead of implementation; TASK-014 then coded Temperature, Tint and the neutral picker against this document, unguessed. Production lives in `crates/luxforge-core/src/modules/basic/white_balance.rs` (the pointwise unit and the solver) and `modules/basic/mod.rs` (the parameters and the `neutral-sample` query), and is checked against the independent reference below within the pointwise contract's default `1e-6 + 1e-6 × |reference|`. See [Basic and histogram](basic-and-histogram.md) for the surrounding product and integration contract, and [product decisions](../decisions.md#basic-adjustments-and-histogram) for the picker default and rejection rules this document freezes.
...
- Verified in `crates/luxforge-core/tests/reference/white_balance.rs` and `crates/luxforge-core/tests/basic_white_balance_reference.rs`; expected values are in [`fixtures/basic/white-balance-cases.json`](../../fixtures/basic/white-balance-cases.json).
...
- Reference implementation: [`crates/luxforge-core/tests/reference/white_balance.rs`](../../crates/luxforge-core/tests/reference/white_balance.rs), proved by [`crates/luxforge-core/tests/basic_white_balance_reference.rs`](../../crates/luxforge-core/tests/basic_white_balance_reference.rs) and visually reviewed by [`crates/luxforge-core/tests/basic_white_balance_visual_review.rs`](../../crates/luxforge-core/tests/basic_white_balance_visual_review.rs) (ignored, no committed images).
