...
- **The colour-constrained brush is not frozen here.** It will reuse this study's metric and its
  refine mapping, but its coverage is the brush's own profile multiplied by a similarity to the
  colour sampled where the stroke began, and its gesture, its sample and its accumulation are the
  brush's work.
