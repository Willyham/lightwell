...
Status: frozen numerical study (TASK-015 of the [Basic and histogram plan](../../tasks/implementation-basic-histogram.json)). No production code changes here: this document, the independent f64 reference under `crates/lightwell-core/tests/reference/colour.rs` and the fixtures under `fixtures/basic/colour-cases.json` are the oracle a later implementation of the `vibrance` and `saturation` Basic units is checked against. Read it alongside [Basic adjustments and histogram](basic-and-histogram.md), whose "Saturation and vibrance" section and integration contract this study answers, and [product decisions](../decisions.md#basic-adjustments-and-histogram) for the accepted ranges.
...
| `crates/lightwell-core/tests/reference/mod.rs` | Shared private sRGB decode/encode/quantize helpers for numerical-task reference modules, plus `pub mod colour;`. |
| `crates/lightwell-core/tests/reference/colour.rs` | The frozen Oklab conversion, saturation and vibrance reference, with the property tests proved above as `#[test]` functions. |
| `crates/lightwell-core/tests/basic_colour_reference.rs` | Generates and reloads `fixtures/basic/colour-cases.json`. |
| `crates/lightwell-core/tests/basic_colour_visual.rs` | The ignored visual review; writes no committed output. |
| `fixtures/basic/colour-cases.json` | Frozen expected values, reloaded by `basic_colour_reference.rs`. |
