...
Limits: 128 MiB encoded source, 64 MP, 16384 px per side, 512 MiB per evaluated frame, 64 MiB of aggregate float scratch across active renders, 8 pointwise units per colour operation, preview uploads up to 4096 px per side, 100 history rows per page, 8 live clients, 256 buffered events, 1 MiB per request line. Decoder limits are not process or GPU memory limits. The rules and review checklist are in [performance rules](../engineering/performance-rules.md).
