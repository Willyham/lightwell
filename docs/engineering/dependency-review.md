...
Local verification of the reviewed policy passed: policy regression tests, Rust core tests, formatting/lint/repository checks, and the full license/source/advisory command. A negative cargo-deny run withholding the ttf-parser exception failed as expected. Hosted verification passed in [run 35465688171](https://github.com/Willyham/lightwell/actions/runs/35465688171) at commit `c0ad340`: dependency audit, fixtures, all three build/package jobs and Linux headless smoke are green. The audit log records both exact versions, dates and follow-up tasks.
