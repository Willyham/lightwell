...
Local verification of the reviewed policy passed: six policy regression tests, six Rust core tests, formatting/lint/repository checks, and the full license/source/advisory command. A negative cargo-deny run withholding the ttf-parser exception failed as expected. Hosted verification follows in the next CI run; the earlier failure above remains historical evidence.
