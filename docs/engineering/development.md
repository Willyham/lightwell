...
| Authentic RAW editor journey, reopen and resource sampling; `--samples` defaults to 3, a functional run proving the journey rather than a latency distribution — pass an explicit count such as `--samples 30` for a p50/p95 claim | `cargo run --release --locked --package xtask -- raw-editor --manifest FILE --output NEW_DIR [--samples N] [--binary PATH]` |
...
| Process failure checks; macOS measurement, `measure`'s `--samples` defaults to 5 app-cold launches per workload, a functional run rather than a latency distribution — pass an explicit count such as `--samples 30` for a p50/p95 claim | `cargo xtask hardening --binary PATH --output NEW_DIR`, `cargo xtask measure --binary PATH --output NEW_DIR [--samples N]` |
