...
Each of those runs also checks the desktop it ran on. The runner reads the frontmost application through LaunchServices (`lsappinfo`, which needs no Automation permission) immediately before each child starts and again once the launch is over, and writes the pair into its own result as `focus_check`, or `focus_checks` where a run makes several launches: `{"status":"checked","method":"lsappinfo","before":NAME,"after":NAME,"changed":BOOL}`. A run whose frontmost application changed fails, naming both applications, after its own checks have been recorded — so a launch that took the desktop fails the run, and so does switching applications or locking the screen yourself while one is going. When the frontmost application cannot be read the record is `{"status":"unavailable","error":…}` and the run continues, because a check that is missing should be visible rather than silently passing; off macOS it is `{"status":"not-checked"}`.
