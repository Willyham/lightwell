...
| `rendered` | quick plus all 27 smoke scenarios, including the four `mask-*` ones, `gallery` and `controls`, through a bounded pool |
...
is 8 s and varies with how much Cargo has to redo; `rendered` a measured 27-scenario workload with 36 editor launches taking 28 s of wall clock
