...
## Headless test dependency correction

Run 35471983478 failed all three native jobs before compilation: the missing-evidence regression entered `verify_smoke`, which imported optional Pillow before checking result files. Local Python already had Pillow, hiding the dependency. Validate evidence metadata before loading pixel tools, retain a structured failure when pixel dependencies are unavailable, and exercise the missing-evidence regression with Pillow explicitly unavailable. Verify the full Python suite with site packages disabled and with pinned Pillow installed, then run the shared check and push the correction. Do not add an image dependency to basic headless checks or skip the process-failure regression.
