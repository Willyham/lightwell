...
| The Performance section's one-second sampler while it is expanded: 0.3 to 0.6% of one core, mostly the redraw it causes ([performance](../specs/performance.md#performance-section-activity-board-and-resource-counters)) | Accepted: it runs only while someone has opened the section, and the section starts collapsed | Do not sample while collapsed or while the state panel is hidden, and do not shorten the interval |
