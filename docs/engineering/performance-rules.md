...
8. **Idle means asleep.** Blocking receives and blocking `accept` are correct; loops that wake to check a flag are not. The preview and overlay workers wake the desktop through a channel whose subscription exists only while one of them is busy, the 500 ms event sync runs only while an asset is open, and the Performance section's one-second sampler only while that section is expanded with the state panel shown. A new timer is gated the same way and justifies its interval against the idle CPU target.
...
| A RAW temperature or tint release waits for the mosaic to be redeveloped before its exact frame and histogram: about 0.54 s on the Z6 and 1.6 s on the X100VI. The drag itself previews approximately on the developed planes, at exposure's latency | Open | The approximation stays preview-only and labelled ([instant previews](../design/instant-preview.md#a-raw-white-balance-during-a-drag)): no committed render, export, sample or analysis may take it. Its error grows with the gain change and with fine texture, highlights and saturated colour, and is largest on the Air 2S |
...
| A point sample through a spatial layer, on either path, evaluates one tile plus its halo, and one 1/16-per-side reduction of the stage on an estimate cache miss. It runs on the catalog owner, where another client's call waits behind it: about 20 ms with Clarity and 38 ms with Clarity and Dehaze on the Z6 and X100VI, 11 and 34 ms on a 24 MP JPEG ([performance](../specs/performance.md#point-samples-through-a-spatial-layer)). On the RAW path a stack with more than one spatial segment still materializes every one but the last | Declared exception to rule 4, recorded in the [presence design](../design/presence-mixer-vignette.md#presence-the-spatial-primitive); answering the sample off the owner is open | Refusing such samples would break readout and UI/API parity; any fix keeps the sampled byte equal to the rendered byte. The tile's input is pulled serially: on the shared pool its rows wait behind a render that holds the pool, and the owner with them |
| A spatial render whose batch the target holds below the pool's width runs each tile's passes on every worker: with all three Presence fields at 60 MP and the inlined scalar accessor it takes about 2.88 s, while Dehaze and Texture still compute 5.2 and 4.9 times the stage to carry Clarity's 327 px halo around every 512 px tile ([current kernel measurements](../specs/performance.md#isolated-rendering-kernels), [tile tradeoffs](../specs/performance.md#presence-exact-renders-on-the-generated-jpegs)) | Accepted for the latency (owner, 2026-09-23); larger tiles are an open owner decision | Output bytes and the spatial target stay as they are; larger tiles must keep a sample equal to the rendered byte and weigh the sample's cost on the owner, which grows with the tile |
