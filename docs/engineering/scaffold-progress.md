...
Maintain synthetic golden inputs with asymmetric color/detail content, EXIF 1–8, tagged/untagged sRGB, greyscale, unsupported CMYK/profile, malformed/truncated and oversized headers. Generate 24/60 MP workloads on demand outside tracked files. Record hashes and expected oriented dimensions. Tests must independently inspect headers and decoded geometry, and confirm hashes preserve the corpus.

Repository conventions cover ignored output/private data, LF text, contributor workflow, and portable document/task checks. Checks validate both active plan shapes, dependency graphs, derived waves, file links and external completion gates. The archived plan stays untouched. A local copy of the schema permits checks without access to a personal skill installation; the checker is implemented in Rust xtask.
