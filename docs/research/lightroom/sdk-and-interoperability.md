...
**P.** The useful lesson for Lightwell is to route UI and agent mutations through the same transaction/history service, with typed settings and observable completion. Lightwell's accepted goal is broader: every introduced operation must be programmable, including edit modules and lifecycle actions. The SDK is a comparison, not the target contract.
