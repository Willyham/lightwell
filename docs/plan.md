...
The previously agreed persistent editing loop becomes the subsequent **M1**: reference a JPEG in a catalog, crop/straighten/rotate/flip it, browse/preview/restore its persistent action history, undo/redo, reopen it and export a new JPEG, with an agent able to edit while the GUI is open. Local IPC/MCP, interactive zoom and manual Locate remain M1 requirements; they do not block S0.
