...
Target the **M4 MacBook Pro on macOS arm64 first** for development, with the proposed wgpu Metal path. The first end-to-end build must also have Windows/Linux packages and launch/load evidence for its accepted matrix. A Linux ARM64 VM, including the current Try Omarchy candidate, can supply labelled guest functional checks but does not establish native Linux GPU performance. The owner has selected **GPL-3.0-or-later**; the repository license is applied and the configured dependency audit remains in progress.
...
3. Every editing/catalog capability available to programs and agents, with inspectable state and results.
4. Simple defaults, beautiful software, progressive growth, and meaningful community extension.
...
S0 exposes an Open image action and a selected photo at Fit. No full grid or editing panels are needed. Whether the later editor uses one workspace or separate Library/Edit modes remains an unanswered owner decision. Neutral dark surfaces, strong typography, predictable focus and accessible controls keep attention on the photo. Native dialogs and platform shortcuts should fit each OS even if controls are custom drawn.
...
| S0 — first end-to-end build | Cross-platform image-loading skeleton and development/agent tooling | [Skeleton acceptance](specs/bootstrap.md#acceptance): reproducible setup/check/build commands, three-platform packages and launch/load evidence, logs/state/captures, M4 baseline; TASK-063 closes the gate |
...
| M5 — richer development and extensions | Texture, clarity, dehaze; customizable tools and an external extension proof | Image-quality fixtures, scale-consistent local processing, and one independently authored extension through the documented boundary |
...
- One command service used by the UI and automation; effects and validation live in the core.
...
- A fixed, modular processing pipeline initially. Typed extension contracts precede dynamic third-party loading.
