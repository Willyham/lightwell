...
Choose Crop in the strip under the photograph, or press `R`, to open a draft that changes nothing yet. Changing any control in the section does the same and applies that change: choose 16:9 and the photograph enters crop mode with a 16:9 frame; drag the rail and it opens at that angle. Changes made while the draft is still being prepared ("Preparing the crop's input stage…") are kept and applied in order as soon as it opens. Choosing the chip that is already chosen opens the draft without refitting the frame, and typing the angle the crop already has just closes the box. Nothing is committed until Apply. The section's controls are disabled while a historical state is shown or a request is in flight, and a crop draft cannot start while a slider drag or a mask gesture is open.
