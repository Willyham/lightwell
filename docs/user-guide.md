...
The tools panel is generated from the registered tool modules, one collapsible section per module in registry order. A header shows the module title, an accent dot when a layer of that module is in the current recipe, the module's hint while collapsed and its reset action when it declares one; an unavailable module shows its reason and cannot expand. Number fields are sliders: drag the thumb, or click the value to type. Each shows its value to the precision its module declares — Exposure to hundredths, Temperature in whole steps — and a drag moves in that module's own increment, so the value you commit is exactly the value you were shown. A slider whose setting is a whole edit on its own shows the change on the photograph while you drag and records one history entry when you let go; the rest change only the number until you release, key-up or press Enter, which commits once. Invalid text stays editable with the message naming the declared range and commits nothing; double-click a label or the slider itself to reset that one field. Enumerations are segmented controls or chips, colours are three fields, groups carry their own reset, actions are buttons enabled only when the current state is editable and every value they need is valid, and a picker is a button that enters its tool's picking mode and stays lit while you are in it. Tab and Shift+Tab move between fields. A control kind this build cannot draw is shown as an explicit unsupported-control message rather than hidden. Every slider shows the value the displayed history state actually holds, so undo, redo and another client's edit all move it; during a historical preview every control stays visible and disabled, showing that entry's values, and the section names why.
