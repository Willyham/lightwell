...
| History labels, recipe rows, module hints, resets and pickers | Implemented | `summary` templates rendered into stored entry labels (catalog format 4), `recipe.describe`, `hint`, module and group `reset`, canvas `title` and `shortcut`, `picker` controls, `developer` flag; all in `module.list` |
