...
| Controls developer proof and UI/API parity | Implemented | `lightwell.controls` is registered only with `--developer`; one-field `edit.set-controls`, separate `edit.reset-controls`, module-owned sampled curves; exact identity pixels and source-buffer sharing. `text`, `pad` and `string` remain the second slice |
