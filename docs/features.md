...
| Generated control vocabulary | Implemented | Soft-range sliders, number fields, steppers, toggles, segmented/chip/menu choices, colour fields/pickers, multi-channel curves, groups, resets and styled actions with vector icons. Continuous patch gestures draft on the 16 ms tick and commit once; discrete controls commit once; no wheel editing. [UI components](design/ui-components.md) |
| Developer components gallery | Implemented | Title-bar Developer button in debug builds or with `--developer`; ten browsable pages of 63 shared reference states, page chooser, Back/Escape, and `workspace.set` navigation without changing the photo |
