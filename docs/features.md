...
| Generated tools panel: sliders, chips, colour fields, groups, resets, pickers | Implemented | Every control from `module.list`; a slider's rail steps by the parameter's declared step and its value shows the declared decimals, with each dragged value quantized before it is sent; a slider whose one field is a whole request (a field patch, or an action's only parameter) drafts, previews live and commits once on release, and every other slider sends nothing until release; a double-click on the label or the slider resets the field; a declared `picker` control enters its module's canvas pick mode through `workspace.set`; unsupported kinds named; Developer section behind `--developer` |
| History labels, recipe rows, module hints, resets and pickers | Implemented | `summary` templates rendered into stored entry labels (catalog format 4), `recipe.describe`, `hint`, module and group `reset`, canvas `title` and `shortcut`, the `picker` control kind bound to a module's pick canvas, `developer` flag; all in `module.list` |
