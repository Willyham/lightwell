...
| RAW camera profiles | Implemented for 84 models; 16 resource-gated | Validated embedded [JSON camera catalog](design/raw-camera-profiles.md) supplies native admission, recording modes, crop and DNG processing capabilities. The [100-model selection](../fixtures/modern-camera-selection.json) currently has 84 configured models; 16 larger models await the RAW resource decision. Authentic decode/development evidence and representative editor verification are tracked in [modern camera support](design/modern-camera-support.md) |
