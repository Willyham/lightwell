...
The full editor cannot be marked delivered by S0. M1 persistent history/live API and M2 exact transforms are now implemented in normal editor mode while this deterministic S0 evidence contract remains available. Tool modules and crop follow in separate milestones. Export, Locate and MCP remain explicit editor follow-ups in the [editor specification](single-image.md).
