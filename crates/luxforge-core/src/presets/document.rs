...
use super::{
    ...
    file_stem, not_a_preset, preset_name, trimmed, unsupported, validate_settings,
};
...
pub(super) fn read(
    ...
    match value.get("version") {
        ...
        None => return Err(unsupported("the Luxforge preset document has no version")),
    }
    let document: Document = serde_json::from_value(value)
        .map_err(|error| unsupported(format!("malformed Luxforge preset document: {error}")))?;
