...
use luxforge_core::{SensorMosaic, sensor_neutral_gains_mapped};
...
const MAX_BYTES: u64 = luxforge_raw::MAX_SOURCE_BYTES as u64;
    ...
    for (_, x, y) in points {
        ...
        let mapped = |px: u32, py: u32, channel: usize| {
            raw.corrected_sensor_sample_location(px, py, channel)
                .map_err(|e| {
                    luxforge_core::Error::new(luxforge_core::ErrorKind::Validation, e.to_string())
                })
        };
