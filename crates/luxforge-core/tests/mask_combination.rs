...
//!
//! The oracle is `tests/reference/mask.rs`, which shares no code with `luxforge-core`, composed
...
fn temp(name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!(
        "luxforge-mask-combination-{}-{}-{name}",
        std::process::id(),
...
impl Fixture {
    ...
    fn mask_command(
        ...
        parameters: Value,
        request: &str,
    ) -> luxforge_core::mask::commands::MaskCommandResult {
        let command =
...
///
/// The oracle is `tests/reference/mask.rs`, which shares no code with `luxforge-core`: its
