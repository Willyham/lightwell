...
//! independent references in `lightwell-reference`; the compiled kinds against those references
...
fn decoded(source: &SourceImage) -> LinearImage {
    let pixels: Vec<[f64; 3]> = source
        ...
        .chunks_exact(4)
        .map(|pixel| {
            std::array::from_fn(|channel| lightwell_reference::srgb_to_linear(pixel[channel]))
        })
        .collect();
