...
fn refusal(payload: serde_json::Value) -> (lightwell_core::ErrorKind, String) {
    let mut mask = Mask::new("Mask 1");
    mask.components.push(Component::new(
        "Radial 1",
        ComponentMode::Add,
...
fn radial_validation_errors_name_the_field() {
    ...
    for (payload, message) in cases {
        ...
        assert_eq!(kind, lightwell_core::ErrorKind::Validation);
...
fn the_kind_table_claims_radial() {
    assert!(lightwell_core::mask::knows_component_kind("radial"));
    ...
    lightwell_core::mask::validate_component_kinds(&mask).unwrap();
}
