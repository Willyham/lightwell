...
fn the_host_seeds_a_limited_stroke_from_the_masked_operations_input() {
    ...
    let ids = lightwell_core::path::references(&component.payload, "the brush component")
    ...
    let recipe = fixture.recipe();
    let ids = lightwell_core::path::references(
...
fn pixel_of(fixture: &Fixture, mask: &lightwell_core::MaskId, value: f64, horizontal: bool) -> u32 {
    let entry = fixture
        .service
        .state(&fixture.asset)
        .unwrap()
...
fn the_hosts_canvas_pick_runs_a_host_query_into_a_host_command() {
    let kinds: Vec<&str> = lightwell_core::mask::sampling_kinds().collect();
    ...
    for (kind, pick) in kinds.iter().zip(commands::canvas()) {
        let lightwell_core::CanvasInteraction::SampleApply {
            query,
            x,
            y,
            action,
