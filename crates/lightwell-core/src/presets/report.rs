//! The per-setting import report: what was carried, what was neutral, what Lightwell cannot
...
pub struct ImportReport {
    /// `lightwell`, `lightroom-xmp` or `lightroom-template`.
    pub format: String,
    ...
    /// Settings Lightwell has no tool for, at a value that changes nothing, so nothing is lost.
    pub neutral: Vec<ReportedSetting>,
    /// Effects Lightwell does not reproduce, because it has no such tool.
    pub unsupported: Vec<ReportedSetting>,
    /// Values Lightwell has a related control for but cannot carry: out of range, from another
    /// process version, disabled in the preset or without a calibrated conversion.
    pub refused: Vec<ReportedSetting>,
}

/// One value transfer: the same number on the Lightwell control with the same name, range and
/// direction. It is not a claim that Lightwell renders what Lightroom renders.
#[derive(Clone, Debug, PartialEq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
