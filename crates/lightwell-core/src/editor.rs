use crate::{
    ...
    Mutation, PreviewJob, PreviewSource, Raster, Snapshot, SnapshotId, Transform,
    modules::{
...
impl EditorService {
    ...
    pub fn preview_job(
        ...
        let preview_source = match source {
    ...
    pub fn sample_entry(
        ...
        let state = self.state(asset_id)?;
        let entry = self.entry(asset_id, entry_id)?;
        let prepared = self.verified_prepared(&state.asset)?;
        let sampled = match &prepared {
            PreparedSource::Jpeg(source) => {
                validate_source_recipe(&state.asset, &entry.snapshot.recipe)?;
                sample(&self.registry, source, &entry.snapshot.recipe, x, y)?
            }
            PreparedSource::Raw(raw) => {
                let settings = raw_settings(raw, &entry.snapshot.recipe)?;
                sample_linear(
        ...
        Ok(PixelSample {
            ...
            source_fingerprint: state.asset.fingerprint,
            width: sampled.width,
    ...
    pub fn apply_action(
        ...
        let sampler = |x: u32, y: u32| -> Result<Option<[u8; 4]>, Error> {
            match &source {
                PreparedSource::Jpeg(_) => Ok(jpeg_evaluation
                    .as_ref()
                    .expect("JPEG evaluation")
                    .pixel(x, y)),
                PreparedSource::Raw(raw) => Ok(sample_linear(
                    &registry,
                    raw.linear.as_ref().ok_or_else(|| {
                        Error::new(ErrorKind::PreparationRequired, "RAW development required")
                    })?,
        ...
        let sample_before = |index: usize, x: u32, y: u32| -> Result<Option<[u8; 4]>, Error> {
            ...
            match &source {
                PreparedSource::Jpeg(image) => {
                    Ok(Evaluation::over_layers(&registry, image, layers)?.pixel(x, y))
                }
                PreparedSource::Raw(raw) => {
                    let prefix_recipe = crate::Recipe {
...
mod tests {
    ...
    fn raw_import_wb_history_redevelopment_and_reopen_preserve_original() {
        ...
        let original = service.preview_job(&initial.asset.id, None, None).unwrap();
        ...
        assert_eq!(
            service
                .preview_job(&initial.asset.id, None, None)
                .unwrap_err()
                .kind,
            ErrorKind::PreparationRequired
        );
