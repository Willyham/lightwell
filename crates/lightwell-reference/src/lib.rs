...
//! against: the sRGB transfer function and exposure here, and one module per study (colour, mask,
//! mixer, Presence, range, tone, vignette, white balance).
//!
//! Every reference is written directly from its study's formulas — the standard sRGB transfer
//! function (threshold 0.04045 / 0.0031308, 12.92, 1.055, 0.055, exponent 2.4) and each design's
//! numerical contract — not by reading or copying the production code. This crate depends on
//! nothing, and `cargo xtask check-repository` refuses a manifest of it that names
//! `lightwell-core`, so a reference cannot import the code it checks; the two implementations only
