//! [`JsonProcess`], one `lightwell-json` process driven over its standard input and output the way an
//! agent drives it: each call writes one request line and reads its answer, so a test can wait on
//! jobs between requests. A test names the binary (`env!("CARGO_BIN_EXE_lightwell-json")` is only
...
/// One running `lightwell-json` process. Dropping it without [`JsonProcess::finish`] kills it.
