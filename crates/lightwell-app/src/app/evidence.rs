...
fn parse_step(step: &Value) -> Result<Step, String> {
    ...
    match kind {
        ...
        other => Err(format!(
            "unknown step kind {other}; expected api, draft, slider, controls, picker, curve, group, section, gallery, tools_scroll, slider_draft, field, reset, pick, view, workspace, preview, palette, hover or capability"
        )),
    }
}
