...
pub(crate) struct Editor {
    ...
    pub(crate) photo: Option<luxforge_ui::PhotoRaster>,
...
impl Editor {
    ...
    fn scratch_summary() -> Value {
        let budget = luxforge_core::ScratchBudget::default();
        json!({
    ...
    fn surface_summary(&self) -> Value {
        json!({
            ...
            "version": self.photo.as_ref().map(luxforge_ui::PhotoRaster::version),
    ...
    fn presented_exact_raster(&self) -> Option<&Arc<luxforge_core::Raster>> {
        self.raster
            .as_ref()
    ...
    fn poll_preview(&mut self) -> Option<luxforge_core::PreviewResult> {
        loop {
    ...
    fn present(&mut self, upload: Upload, raster: &luxforge_core::Raster) {
        ...
        self.photo = luxforge_ui::PhotoRaster::new(
            raster.rgba.clone(),
            raster.width,
            raster.height,
            self.photo_version,
    ...
    fn overlay_source(&self) -> Option<(u64, &Arc<luxforge_core::Raster>, bool)> {
        let generation = self.presented_generation;
    ...
    fn show_entry(&mut self, entry: luxforge_core::EntryId) {
    ...
    fn sequence_of(&self, entry_id: &luxforge_core::EntryId) -> Option<u64> {
        self.history
            .entries
            .iter()
    ...
    pub(crate) fn add_mask_target(&self, action: &str, request: &mut Map<String, Value>) {
        ...
        if self.maskable_action(action) {
            request.insert(luxforge_core::MASK_FIELD.to_owned(), json!(mask));
        }
    }
    ...
    fn settle_draft(&mut self, revision: u64, entry: &luxforge_core::EntryId) {
...
mod tests {
    ...
    fn gallery_uses_session_state_without_a_photo_and_respects_developer_mode() {
        ...
        assert_eq!(
            ...
            luxforge_core::COMPONENT_GALLERY_PAGE_COUNT
        );
            ...
            for (index, control) in module.controls.iter().enumerate() {
                ...
                let mut own: Vec<&str> = controls
                    ...
                    .filter_map(|control| match control {
                        luxforge_core::Control::Number { parameter, .. } => {
    ...
    fn raw_fields_show_the_displayed_entrys_described_values() {
        ...
        adjusted.gains =
            luxforge_core::gains_from_temperature_tint(3500.0, 12.0, Z6_CAM_XYZ).unwrap();
