    ...
    fn record(&self) -> Value {
        let (key, value) = match &self.action {
            ...
            CapabilityAction::Secret { field, profile, .. } => (
                "secret",
                with_profile(
                    json!({"field": field, "value": luxforge_core::capabilities::redact::REDACTED}),
                    *profile,
                ),
            ),
    ...
    pub(crate) fn record(&self) -> Value {
        match self {
            ...
            Self::Api { method, params } => json!({"api":{
                "method": method,
                "params": luxforge_core::redact_params(method, &Value::Object(params.clone())),
            }}),
...
mod tests {
    ...
    fn the_slider_field_and_reset_steps_round_trip_their_scripts() {
        ...
        assert_eq!(
            steps[7].record(),
            json!({"reset":{"module":"luxforge.basic","group":"Tone"}})
        );
    ...
    fn a_pick_step_round_trips_its_rendered_coordinates() {
        ...
        for refused in [
            r#"[{"pick":{"x":120}}]"#,
            r#"[{"pick":{"x":120,"y":-2}}]"#,
            r#"[{"pick":{"x":120,"y":80,"mode":"luxforge.basic"}}]"#,
        ] {
