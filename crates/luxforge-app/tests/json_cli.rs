...
fn subprocess_client_edits_queries_and_exits_cleanly_on_eof() {
    ...
    let fixture = Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .canonicalize()
        .unwrap();
    let mut child = Command::new(env!("CARGO_BIN_EXE_luxforge-json"))
...
/// One interactive `luxforge-json` process: each call writes one request line and reads its
...
fn a_proof_endpoint_client_installs_activates_runs_the_task_and_applies_its_tint() {
    ...
    let endpoint = luxforge_core::ProofEndpoint::start(&key).unwrap();
    let data_root = std::env::temp_dir().join(format!(
        "luxforge-json-cli-proof-{}-{}",
        std::process::id(),
        NEXT.fetch_add(1, Ordering::Relaxed)
    ));
