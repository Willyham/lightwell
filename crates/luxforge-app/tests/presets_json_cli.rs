...
fn temp_catalog() -> PathBuf {
    let path = std::env::temp_dir().join(format!(
        "luxforge-presets-json-cli-{}-{}.sqlite",
        std::process::id(),
...
/// One `luxforge-json` process over one catalog, answering one request per line.
struct JsonClient {
    child: Child,
...
impl JsonClient {
    ...
    fn finish(mut self) {
        ...
        assert!(self.child.wait().expect("luxforge-json exits").success());
    }
}
