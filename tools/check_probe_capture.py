...
def check(path):
    ...
    # Orientation 6: blue/red over gold/green. Match saturated interiors, excluding UI/text.
    colors = [(40,70,220),(220,35,45),(235,195,30),(35,190,65)]
    xs, ys = [], []
    ...
    assert abs(width/height - 2/3) < 0.015, 'Incorrect Fit aspect ratio'
    ...
    for (fx,fy), expected in zip([(0.25,0.25),(0.75,0.25),(0.25,0.75),(0.75,0.75)], colors):
        ...
        actual.append(pixel)
    return {'status':'passed', 'physical_size':image.size, 'image_bounds':[left,top,right,bottom], 'corner_rgb':actual, 'tolerance_per_channel':8, 'scope':'orientation-6 Fit geometry and sRGB interiors; not monitor calibration or native dialog verification'}
