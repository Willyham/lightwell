...
fn exceptions() -> Value {
    json!([
 {"id":"RUSTSEC-2024-0436","package":"paste","version":"1.0.15","reviewed":"2026-09-19","expires":"2026-12-18","task":"TASK-065","reason":"Build-time macro in pinned Metal dependency; no supported Iced upgrade removes it. Local S0 development only."},
 {"id":"RUSTSEC-2026-0192","package":"ttf-parser","version":"0.25.1","reviewed":"2026-09-19","expires":"2026-10-19","task":"TASK-066","reason":"Pinned Iced system/bundled-font stack; no application font import. Undisclosed upstream report requires short review window; no distribution approval."}])
}
fn validate(
    ex: &Value,
    packages: &Value,
...
pub fn checked(root: &Path) -> Result<(String, String)> {
    ...
    let tasks = read_json(&root.join("tasks/implementation.json"))?;
    let config = validate(
        &exceptions(),
        &data["packages"],
        &tasks["tasks"],
